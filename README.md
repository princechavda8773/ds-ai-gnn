...
|---|---|---|---|
| 15 min | `python step0_make_dataset.py` | Builds a synthetic social graph + simulates an action cascade spreading through it (like Digg/Weibo, but generated locally) | §2.6 (f), §3 Dataset |
| 20 min | `python step1_ego_sampling_and_features.py` | For every user, extracts their local ego-network (BFS, 2 hops) + computes handcrafted features (degree, clustering, active-neighbor ratio, etc.) | §2.1 (a), §2.4 (d) |
| 10 min | `python step2_model.py` | Defines the model (just prints the architecture to sanity check it — nothing trains here) | §2.2 (b), §2.3 (c), §2.5 (e) |
| 20 min | `python step3_train.py` | Trains the full model + 3 baselines + 2 ablations, prints a results table, saves weights | §2.7 (g), §4 Baselines, §5 Evaluation |
| remaining time | Read the code, tweak hyperparameters, re-run, write up REPORT.md in your own words | §6 Deliverables |
...
1. **Open `step0_make_dataset.py` first.** Skip the code, just read the giant
...
2. **Open `step1_ego_sampling_and_features.py`.** This is literally "cut a
...
3. **Open `step2_model.py`.** Read `SocialInfluenceModel.forward()` line by
...
4. **Open `step3_train.py`.** This is standard PyTorch: loop over batches,
...
This uses **synthetic** data (see the docstring in step0 for why), so don't
...
- **Try GAT instead of GCN**: set the `GNN_TYPE` environment variable before
  running step 3:
  ```bash
  GNN_TYPE=gat python step3_train.py
  ```
  (On Windows PowerShell: `$env:GNN_TYPE="gat"; python step3_train.py`)
...
- **k-hop sensitivity sweep (§5 in the PS)**: runs the whole pipeline for
  ...
  ```bash
  python step4_sensitivity_sweep.py
  ```
...
- **Swap in a real dataset**: only `step0_make_dataset.py` needs to change —
  ...
  `data/labels.csv` (`node,label` columns). Steps 1–4 don't need to change
  at all. Public options: SNAP's Digg/Weibo cascades, or OAG.

## 5. Files

```
step0_make_dataset.py               synthetic graph + cascade simulation
step1_ego_sampling_and_features.py  ego networks + handcrafted features
step2_model.py                      GNN encoder + prediction head (model definitions)
step3_train.py                      training, baselines, ablations, evaluation
data/                                generated graph/labels/instances (created by steps 0-1)
outputs/full_model.pt                trained weights (created by step 3)
